...
aws s3 cp s3://$S3_BUCKET_CONF/hive/config/udf/hive-geoip.jar /usr/local/spark/jars/ > /dev/null
aws s3 cp s3://$S3_BUCKET_CONF/hive/config/udf/geoip-api.jar /usr/local/spark/jars/ > /dev/null
env | grep SPARK | awk '{print "export \"" $0 "\""}' > /usr/local/spark/conf/spark-env.sh
export CLASSPATH="/usr/local/spark/jars/mysql-connector-java.jar"
